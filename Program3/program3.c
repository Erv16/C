...
void f1(){
    ...
    free(a); //Free up the memory allocation for pointer a
    a = NULL; //Assigning the pointer to a null 
}
void f2(){
    int a[10];
    ...
    for (int i = 10; i >0; i--){ //Rectified the loop to access memory correctly
        printf("%d ", a[i]);
    }
    printf("\n");
}
...
void f3(){
    char * cptr = calloc(11, sizeof(char)); //Increased size by 1 to accomodate the terminal character
    cptr[10] = 'a';
    char * junk = &cptr[0];
    free(junk);
}
